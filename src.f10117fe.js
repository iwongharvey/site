...
exports.NEXT_FEATURED_PROJECT = exports.LOAD_PROJECTS_ERROR = exports.RECEIVE_PROJECTS = exports.PARSE_PROJECTS = exports.LOAD_PROJECTS = exports.TOGGLE_NAV_GROUP = exports.ROUTE_TO = exports.RESIZE = void 0;
const RESIZE = 'resize';
...
exports.JOIN = exports.CONTACT = exports.GROUP = exports.ABOUT = exports.NEWS = exports.NEWS_LIST = exports.WORK_GALLERY = exports.WORK_DETAIL = exports.WORKS_LIST = exports.HOME = void 0;
const HOME = {
  id: "home",
  match: ["home"]
};
...
const navItemGroup = ({
  ui,
  bus
}, item) => ['li', ui.nav.item, ['div', [_hdomComponents.appLink, item.active ? ui.nav.linkActive : ui.nav.link, () => bus.dispatch([ev.TOGGLE_NAV_GROUP, item.name]), item.name], item.open ? ['ul', ui.nav.subContent, ...(0, _transducers.map)(item => [navItem, item], item.children)] : null]];

const navItem = ({
  ui
}, item) => ['li', ui.nav.item, [_routeLink.routeLink, item.active ? ui.nav.linkActive : ui.nav.link, item.route, item.name]];

const navContent = ({
  ui,
  views
}) => ['ul', ui.nav.content, ...(0, _transducers.map)(item => [item.hasOwnProperty('children') ? navItemGroup : navItem, item], views.nav.deref().items)];
...
exports.nav = nav;
},{"@thi.ng/hdom-components":"node_modules/@thi.ng/hdom-components/index.js","@thi.ng/interceptors":"node_modules/@thi.ng/interceptors/index.js","@thi.ng/transducers":"node_modules/@thi.ng/transducers/index.js","../events":"src/events.ts","./route-link":"src/components/route-link.ts"}],"src/components/root.ts":[function(require,module,exports) {
"use strict";
...
const logo = ({
  ui
}) => [_routeLink.routeLink, ui.logo, [routes.HOME.id, null], 'Alternability'];

const header = ({
  ui
}) => ['div', ui.header, logo, _nav.navSwitch];

function root({
  ui,
  views
}) {
  return ['div', ui.root, views.routeComponent, header, _nav.nav];
}
},{"../routes":"src/routes.ts","./route-link":"src/components/route-link.ts","./nav":"src/components/nav.ts"}],"src/app.ts":[function(require,module,exports) {
"use strict";
...
class App {
  ...
  start() {
    ...
    (0, _rstream.fromEvent)(window, 'resize').subscribe({
      next() {
        bus.dispatch([ev.RESIZE, window.matchMedia('(min-width: 30em)').matches]);
      }
...
const featuredProjects = ({
  ...
  bus.dispatchLater([ev.NEXT_FEATURED_PROJECT], 4000);
  const pid = views.featured.deref()[views.currentFeatured.deref()];
  const projects = views.projects.deref();
  return ['div', ui.gallery, [_routeLink.routeLink, {}, [routes.WORK_DETAIL.id, {
    id: pid
  }], ['img', Object.assign({}, ui.homeGalleryImg, {
    src: (0, _utils.resource)(projects[pid]['images'][0])
  })]]];
};
...
function join({
  ...
  return ['div', ui.join, ['div', ui.contentOne, ['div', ui.title, 'Join Us']]];
}
},{}],"src/components/work-spec.ts":[function(require,module,exports) {
"use strict";
...
const workSpec = ({
  ui
}, project) => ['div', ['div', ui.project.title, project.title], ['div', ui.project.spec, 'Client:', project.client, 'Area:', project.area, 'Category:', project.category, 'Designer:', project.designer, 'Members:', project.members, 'Construction:', project.construction, 'Constructor:', project.constructor, 'Design Date:', `${project.date.getFullYear()}-${project.date.getMonth()}`, 'Construction Date:', `${project.date.getFullYear()}-${project.date.getMonth()}`, 'Location:', project.location]];

exports.workSpec = workSpec;
},{}],"src/components/work-list.ts":[function(require,module,exports) {
"use strict";
...
const projectThumb = ({
  ...
  return ['div', ui.project.thumbContainer, [_routeLink.routeLink, {}, route, ['img', Object.assign({}, ui.project.thumbImg, {
    src: (0, _utils.resource)(project.thumbnail)
...
const projectGroup = ({
  ...
  const projects = views.projects.deref();
  return ['div', ui.project.group, ['div', ui.project.category, name], ['ul', ui.project.thumbList, ...(0, _transducers.map)(pid => ['li', ui.project.thumbItem, [projectThumb, projects[pid]]], pids)]];
};
...
const projectInfo = ({
  ...
  return ['div', ui.project.info, [_workSpec.workSpec, project], ['div', ...(0, _transducers.mapIndexed)((i, _) => [projectImage, project, i], project.images)]];
};
...
const projectDetail = ({
  ...
  return project ? ['div', ui.workDetail, [projectInfo, project]] : null;
};
...
function workList({
  ...
  return () => {
    return ['div', ui.workList, ['div', ui.workListThumb, ...(0, _transducers.map)(([name, pids]) => [projectGroup, name, pids], (0, _transducers.pairs)(views.projectGroups.deref()))], [projectDetail, views.project.deref()]];
  };
}
},{"@thi.ng/transducers":"node_modules/@thi.ng/transducers/index.js","./route-link":"src/components/route-link.ts","../utils":"src/utils.ts","../routes":"src/routes.ts","./work-spec":"src/components/work-spec.ts"}],"src/components/gallery.ts":[function(require,module,exports) {
"use strict";
...
const galleryImage = ({
  ...
  if (project) {
    ...
    return ['div', ui.gallery, [_routeLink.routeLink, {}, route, ['img', Object.assign({}, ui.galleryImg, {
      src: (0, _utils.resource)(images[index])
    })]], ['div', ui.galleryInfo, [_workSpec.workSpec, project]]];
  }
};
...
const getNavItems = route => [...(0, _transducers.map)(item => {
  ...
  if (isNavItemGroup(item)) {
    ...
    for (let i = 0; i < item['children'].length; i++) {
      ...
      child.active = child.match(route);

      if (!matched) {
        matched = child.active;
      }
    }

    item['open'] = item['active'] = matched;
  } else {
...
const toggleGroup = name => items => [...(0, _transducers.map)(item => {
  if (item['name'] == name) {
    item['open'] = !item['open'];
  }
...
const parseProjects = projects => [...(0, _transducers.map)(proj => {
  proj['date'] = new Date(Date.parse(proj['date']));
  proj['construction_date'] = new Date(Date.parse(proj['construction_date']));
  return proj;
}, projects)];
...
const CONFIG = {
  router: {
    ...
    routes: [routes.HOME, routes.WORKS_LIST, routes.WORK_DETAIL, routes.WORK_GALLERY, routes.CONTACT, routes.JOIN]
  },
  events: {
    [_router.EVENT_ROUTE_CHANGED]: [(0, _interceptors.valueSetter)("route"), (state, [_, route]) => {
      ...
      if (route.id == routes.WORK_DETAIL.id) {
        state1 = (0, _paths.setIn)(state, 'project', route.params.id);
      } else if (route.id == routes.WORK_GALLERY.id) {
        const s = (0, _paths.setIn)(state, 'project', route.params.id);
        state1 = (0, _paths.setIn)(s, 'galleryImage', parseInt(route.params.index));
      }
    ...
    [ev.TOGGLE_NAV_GROUP]: (state, [_, name]) => ({
      [_interceptors.FX_STATE]: (0, _paths.updateIn)(state, 'nav.items', toggleGroup(name))
    }),
    ...
    [ev.RECEIVE_PROJECTS]: (state, [_, projects]) => {
      ...
      const state3 = (0, _paths.setIn)(state2, 'featured', featured);
      return {
        [_interceptors.FX_STATE]: (0, _paths.setIn)(state3, 'initialized', true)
      };
    },
    ...
    [ev.NEXT_FEATURED_PROJECT]: (state, _) => {
      ...
      return {
        [_interceptors.FX_STATE]: (0, _paths.updateIn)(state, 'currentFeatured', v => featured.length > 0 ? (v + 1) % featured.length : 0)
      };
    }
  },
  effects: {},
  ...
  ui: {
    app: {
      class: 'sans-serif f6 relative dark-gray lh-title h-100'
    },
    link: {
      class: 'link'
    },
    ...
    header: {
      class: 'pa3 header relative'
    },
    logo: {
      class: 'f3 absolute v-center link'
    },
    nav: {
      container: {
        class: 'nav relative'
      ...
      switch: {
        class: 'right-1 link absolute f5 dn-ns v-center'
      },
      content: {
        class: 'ph3 ma0 w3 list pa0'
      },
      item: {
        class: 'mv1'
      },
    ...
    homeGalleryImg: {
      class: 'absolute cover vh-100 top-0 left-0 fade-in-out'
    },
    galleryImg: {
      class: 'absolute cover vh-100 top-0 left-0'
    },
    galleryInfo: {
      class: 'absolute bottom-1 right-2 mw-70'
    },
    workList: {
      class: 'content grid dg h-100 w-100 absolute'
    },
    workListThumb: {
      class: 'content-left pl2 mt1-ns'
    },
    workDetail: {
      class: 'content-right overflow-scroll'
    },
    project: {
      group: {
        class: 'mb2'
      ...
      thumbContainer: {
        class: 'relative thumb-container'
      },
      thumbTitle: {
        class: 'no-underline absolute vh-center tc dn thumb-title white nus'
      },
      thumbImg: {
        class: 'db thumb-img'
      },
      ...
      title: {
        class: 'f5 mb5-ns mb3 mt0 mt1-ns'
      },
      spec: {
        class: 'dg project-spec f7 mb3'
      },
      image: {
        class: 'db img mb1'
      }
    ...
    contentOne: {
      class: 'content-one pa3 pa0-ns pl2-ns mt1-ns'
    },
    join: {
      class: 'content grid dg'
    },
...
},{"@thi.ng/interceptors":"node_modules/@thi.ng/interceptors/index.js","@thi.ng/transducers":"node_modules/@thi.ng/transducers/index.js","@thi.ng/paths":"node_modules/@thi.ng/paths/index.js","@thi.ng/router":"node_modules/@thi.ng/router/index.js","./routes":"src/routes.ts","./effects":"src/effects.ts","./events":"src/events.ts","./components/home":"src/components/home.ts","./components/contact":"src/components/contact.ts","./components/join":"src/components/join.ts","./components/work-list":"src/components/work-list.ts","./components/gallery":"src/components/gallery.ts","./utils":"src/utils.ts"}],"src/index.ts":[function(require,module,exports) {
"use strict";
